...
public class SentencePresenter implements SentenceContract.Presenter {
    ...
    public void getWriting(int page) {
        Observable<List<SentenceBean>> observable = Observable.create(new ObservableOnSubscribe<List<SentenceBean>>() {
            @Override
            //将事件发射出去,持有观察者的对象
            public void subscribe(final ObservableEmitter<List<SentenceBean>> emitter) throws Exception {
                    ...
                    public void onResponse(Call call, Response response) {
                        if (response.isSuccessful()) {
                            ...
                            try {
                                ...
                                List<SentenceBean> musicBeen = gson.fromJson(jsonObject.getString("T1348649580692"),
                                        new TypeToken<List<SentenceBean>>() {}.getType());
                                // 获取数据成功
        ...
        ObservableDecorator.decorate(observable).subscribe(new Observer<List<SentenceBean>>() {
            @Override
            ...
            @Override
            public void onNext(List<SentenceBean> musicBeen) {
